...
**归属**：Capability tree 描述 **Host 如何发现与编排可执行能力**（tool / skill / route）。它与 AIPP 的 `GET /api/tools`、`GET /api/skills` 对齐，但是 **World One 侧的导航图**。
...
```json
{
  ...
  "kind": "route",
  "title": "菜谱管理",
  "description": "菜谱、食材、烹饪步骤。",
  "aap_pre": "用户提到菜、菜谱、食材、烹饪时使用本应用。",
  "children": [
    {
...
- Host 实现参考：World One `CapabilityTreeService`、`CapabilityTreeController`
