...
For an AIPP that uses the Host-provided `sys.app-info` main widget and has no app-owned widget,
replace the widgets structure/main-widget checks with:
...
| `assertValidEffectIdentityField` | optional boolean `effect_identity` on client tools (auto-run by `assertValidToolsApiStructure`; see `effect-identity.md`) |
| `AippFunctionAuthoritySpec` | Provider-neutral function-authority wire names; provider tests validate behavior and ownership |
...
| Widget 非 `sys.*` | ✅ | `render` |
| **每个 app** | ✅ | **恰好一个 `is_main:true` app-owned widget**；若 `main_widget_type` 为 Host 提供的 `sys.app-info`，可为零 |
