...
> 受众：**开发者 / LLM**。
>
> **Coding agents（Codex / Claude Code / Cursor）**：不要整篇加载本文。先读 [`AGENTS.md`](AGENTS.md) → [`spec/INDEX.md`](spec/INDEX.md) 按需打开一篇 spec；Tier 0 粘贴块见 [`docs/tier0-bootstrap.prompt.md`](docs/tier0-bootstrap.prompt.md)。
>
> **需要全文时**：人类读者或排查疑难时可通读本文。
...
- **领域提示：** `/api/tools` 根级只用 `prompt_contributions`（`layer: aap_pre` / `aap_post`）；根级 `system_prompt` 已移除（Host 仅打日志，不注入）。
...
- **Tool placement（v3）：** 用顶层 `visibility` + 可选 `owner_widget` / `router_shortcut` / `mutates_display`；嵌套 `scope.level` / `visible_when` 仅作 legacy 兼容。
...
> **渐进式发现**：实现任务时优先 [`AGENTS.md`](AGENTS.md) + [`spec/INDEX.md`](spec/INDEX.md) + 单篇 `spec/*.md`；合规门禁见 [`spec/verify.md`](spec/verify.md)。仅当 INDEX 指向本文 §N 时再读对应章节，避免整篇进 context。
...
7. **Host 系统 widget（`sys.*`）**：哪些能在 tool 响应里返回、JSON 长什么样 — [`spec/system-widgets.md`](spec/system-widgets.md)
8. **Capability tree**：Host 如何发现你的 tool/skill（与 widget 目录分离）— [`spec/capability-tree.md`](spec/capability-tree.md)
...
| 层级 | 文件 | 用途 |
|------|------|------|
| Tier 0 | [`docs/tier0-bootstrap.prompt.md`](docs/tier0-bootstrap.prompt.md) | 粘贴到 Cursor / Codex / Claude Code 配置（~500 tokens） |
| Tier 1 | [`AGENTS.md`](AGENTS.md) | Agent 进仓库后首先阅读 |
| Tier 2 | [`spec/INDEX.md`](spec/INDEX.md) | 按任务路由到单篇 spec |
| Tier 3 | `spec/*.md`, `docs/quickstart-checklist.md` | 主题深度 |
| Tier 4 | 本文 `README.md` §N | 百科全书，按需切片 |

### Spec 文档索引（Host 与跨 app 约定）

| 文档 | 内容 |
|------|------|
| [`spec/INDEX.md`](spec/INDEX.md) | **任务路由表**（agent 用） |
| [`spec/field-semantics.md`](spec/field-semantics.md) | **易错字段设计说明**（placement / refresh / 副作用） |
| [`spec/verify.md`](spec/verify.md) | **`assert*` 合规门禁** |
| [`docs/quickstart-checklist.md`](docs/quickstart-checklist.md) | 新 app 勾选清单 |
| [`spec/widgets.md`](spec/widgets.md) | Widget manifest、ESM、`hostApi`、views/`refresh_tool`/theme |
| [`spec/tool-responses.md`](spec/tool-responses.md) | `POST /api/tools`、`_context`、响应优先级 |
| [`spec/skills.md`](spec/skills.md) | Skill 索引、SKILL.md、Tool 边界 |
| [`spec/sessions.md`](spec/sessions.md) | `session_policy`、`new_session` |
| [`spec/host-decoupling.md`](spec/host-decoupling.md) | lifecycle、tool placement、`refresh_tool`/`mutates_display`、events、prompts |
| [`spec/events.md`](spec/events.md) | `POST /api/events` |
| [`spec/host-registration.md`](spec/host-registration.md) | Host 注册与冒烟 |
| [`spec/system-widgets.md`](spec/system-widgets.md) | `sys.*` 类型表、`data` schema、AIPP 何时可返回 |
| [`spec/capability-tree.md`](spec/capability-tree.md) | 能力树节点、`GET /api/capability-trees/*`、与 tools/skills/widgets 关系 |
| [`spec/configuration.md`](spec/configuration.md) | `sys.configuration` 与 `/api/configuration` |
| [`spec/host-injection.md`](spec/host-injection.md) | `PUT /api/host/bindings` |
| [`spec/host-url.md`](spec/host-url.md) | Host URL 解析 |
| [`spec/decision-reactor-invoke.md`](spec/decision-reactor-invoke.md) | 决策反应调用 |
| [`spec/ontology-world-catalog.md`](spec/ontology-world-catalog.md) | 本体世界目录 |

> **写完怎么自查**：[`spec/verify.md`](spec/verify.md) — 把响应 JSON 喂给 `AippAppSpec` / `AippWidgetSpec` 的 `assert*`（亦见 §15）。文档与方法不一致时，**以方法为准**。
...
### 3.3 Tool placement（`visibility` + 可选字段）

`/api/tools` 中每个 tool 必须携带 `visibility`；其余为可选 placement / 副作用字段（v3+ 推荐**扁平**声明，勿再嵌套 `scope`）：
...
| 字段 | 含义 |
|------|------|
| `visibility` | **谁**可调：`llm`（Agent 循环）、`ui`（widget `hostApi`）、`host`（Host 调度器） |
| `owner_widget` | 可选。绑定到某 widget；含 `llm` 的工具**仅在该 canvas 激活时**进入 LLM 列表（主对话 catalog 排除） |
| `router_shortcut` | 可选。主对话 Router 可 one-hop 直达（替代 legacy `scope.level=universal`） |
| `mutates_display` | 可选。此工具可能使 widget 展示过期；Host 在 LLM 未主动 refresh 时可兜底调用 widget 的 `refresh_tool` |
| `catalog_manual` | 可选 `boolean`；为 `true` 时 Host 能力目录单独展示。**仅 exposure 标记**，不影响 LLM 可见性 |

**Legacy `scope` 对象**（`level` / `owner_app` / `visible_when`）Host 仍会在 ingest 时归一化为上述字段；新 app **不要**再写 nested `scope`。

Widget manifest 上的 `scope.tools_allow` / `tools_deny` 是**运行时视图过滤**，与 tool placement 无关。详见 [`spec/host-decoupling.md`](spec/host-decoupling.md) §7–§9、[`spec/widgets.md`](spec/widgets.md) §5。
...
> Coding agent 专题：[`spec/skills.md`](spec/skills.md)  
...
> Coding agent 专题：[`spec/widgets.md`](spec/widgets.md)

### 5.1 必需字段

| 字段 | 必选 | 说明 |
|------|------|------|
| `type` | ✅ | 全局唯一标识（如 `recipe-board`），**不得使用 `sys.*` 前缀**（见 [`spec/system-widgets.md`](spec/system-widgets.md)） |
| `app_id` | ✅ | 与 `/api/app.app_id` 一致 |
| `is_main` | ✅ | `true` = app 主入口；**每个 app 必须恰好一个 `is_main:true`** |
| `display_mode` | ✅ | `canvas` \| `chat` \| `pop`；`pop` = 浮窗，不进 chat/canvas 流 |
...
**可选** — 轻量预览（能力地图详情、目录缩略图等）。Host 优先调用 `preview`；未导出时回退到
`mount(targetEl, previewHostApi, { __preview: true, ... })`：

```js
/** Optional — read-only UI shell; no data load, no session/canvas side effects. */
export function preview(targetEl, hostApi, context) {}
```

`context` 形状（Host 传入，widget 只读）：

| 字段 | 说明 |
|------|------|
| `widget_type` | manifest `type` |
| `app_id` | 所属 AIPP |
| `display_mode` | `canvas` \| `chat` \| `pop` |
| `title` | manifest / registry 标题（可选） |
| `description` | manifest 描述（可选） |

预览模式下 `hostApi.preview === true`；`callTool` / `proxyTool` / `invokeAction` 等为
no-op 或返回 `{ ok: true, preview: true }`。Widget 不得在此模式发起会话切换、嵌套挂载
Canvas 或其它 widget。

Host 保护（World One 实现）：预览调用带 **超时**；对缺少 `preview()` 且 `mount` 会递归嵌套的 widget
（如 `sys.capability-tree`）**禁止 mount 回退**；已实现 `preview()` 的 widget 可安全展示静态样例 UI。
...
### 5.3 Host 系统 widget 与能力树（必读）

- **`sys.*`**：Host 内置 UI 协议。AIPP **不注册**，但可在 tool 响应里写 `widget_type: "sys.confirm"` 等。完整列表与 JSON 见 [`spec/system-widgets.md`](spec/system-widgets.md)；Java 常量 `AippSystemWidget`。
- **Capability tree**：Host 侧可执行能力地图（route / tool / skill）。`widgets/` 子树仅为 widget **目录**。API 与节点 schema 见 [`spec/capability-tree.md`](spec/capability-tree.md)。
- **运行时目录**：`GET {host}/api/widgets` 返回合并后的 widget manifest（含 `worldone-system` 的 `sys.*` catalog）；`GET {host}/api/capability-trees/{app_id}` 返回你的 app's 树。
...
"views": [
  {
    "id":       "ALL",
    "label":    "全部",
    "llm_hint": "用户正在查看全部菜谱。如修改了任何菜谱，操作后调用 {refresh_tool} 刷新。"
  }
],
"refresh_tool": "recipe_view"
```

变更类工具在 `/api/tools` 上声明 `mutates_display: true`（与 `owner_widget` 同 widget）。

Widget 前端调用全局函数 `aippReportView(widgetType, viewId)` 上报当前 Tab；Host 在下次 LLM 调用时把对应 `llm_hint` 注入最高优先级 system prompt。`{refresh_tool}` 占位符自动替换（兼容 `{refresh_skill}`）。

`mutates_display` 工具被调用后，若 LLM 没主动 refresh，Host 自动补调一次 `refresh_tool`。
...
```json
{
  ...
  "description":             "菜谱管理面板",
  "entry_tool":              "recipe_view",
  "welcome_message":         "菜谱管理已打开。",
  "widget_prompt":           "用户正在查看和管理菜谱。使用 recipe_create/update/delete 操作。",
  "supports": {
    "disable": true,
    "theme":   ["background", "surface", "text", "textDim", "border",
                "accent", "font", "fontSize", "radius", "language"]
  ...
  "views": [
    { "id": "ALL",     "label": "全部",   "llm_hint": "用户在查看全部菜谱。修改后调用 {refresh_tool}。" },
...
> Coding agent 专题：[`spec/host-decoupling.md`](spec/host-decoupling.md)、[`spec/events.md`](spec/events.md)  
...
**问题**：某些 tool 应由 Host 在每轮对话前/后自动调用（如记忆预加载、记忆固化），而不是等 LLM 想起来再调。

**协议**（挂在 **`/api/tools` 的 tool 条目**上，不是 skill 索引）：
...
| 值 | 含义 | LLM 可见？ |
|----|------|-----------|
| `on_demand`（默认） | LLM / UI / 用户按需调用 | 是（若 `visibility` 含 `llm`） |
| `pre_turn` | 每条用户消息发出后、LLM 思考**之前**，Host 自动调用 | **否** |
| `post_turn` | 一轮对话结束后，Host **异步**自动调用 | **否** |

**典型例子（memory-one）：**

- `memory_load` + `lifecycle: "pre_turn"` + `inject_context.request_context` → Host 预加载记忆进隐藏上下文
- `memory_consolidate` + `lifecycle: "post_turn"` + `inject_context.turn_messages` → Host 把本轮消息写入记忆库

完整时间线、JSON 示例与迁移说明见 [`spec/host-decoupling.md`](spec/host-decoupling.md) §2。

`pre_turn` / `post_turn` tool 应设 `visibility: ["host"]`（或由 app 的 enrich 逻辑从 `lifecycle` 推导）。**只用 `lifecycle`** — `auto_pre_turn` / `background` 已从协议移除。
...
### 5.5 `display_label_zh` — 自描述 UI 标签
...
Host 暴露 `GET /api/tool-labels` 聚合所有 app 的标签字典；前端启动时拉取并缓存。**Host 自己不再维护任何 tool 标签清单。** 旧字段 `display_name` 仅 Host 读兼容，新 manifest 不得出现。

### 5.6 `prompt_contributions` — 自描述领域提示词

**问题**：你的 AIPP 涉及"菜谱/食材/烹饪"等领域词；Host 不能在它的 system prompt 里写这些（它要保持通用）。怎么让 LLM 知道？

**协议**：在 `/api/tools` 顶层声明（**不要**再用根级 `system_prompt`）：
...
### 5.7 World event & widget naming — Session / Event / Widget 三层标题
...
**与 §5.5 的区别**：§5.5 的 `display_label_zh` 仅用于 **tool 在工具字典中的标签**。本节字段用于 **session、world event、widget 卡片**；新协议**不再**用 `display_name` 描述 widget/event/session 标题（旧字段 `tags.display_name` 仅作 Host 读兼容）。
...
#### `session_summary` 解析（生产者，无 hardcode）
...
#### NEED_INPUT → `parameter_missing` event
...
- `html_widget.title`（§10 ChatEvent）与 `widget.title`（本节）语义相同：均为 widget 自设标题；`context_title` 为上游传入的默认来源。
...
**可选 `load` / `refresh` 方法**（用于动态配置项）：

- 在 `configuration.methods` 声明：
...
- `load`：配置弹窗打开时调用，可返回 `values` 与 `field_options`
- `refresh`：任意配置值变化时调用，请求体建议含 `values`、`previous_values`、`changed_bind`
- 若无动态逻辑，AIPP 可 no-op 返回 `{ "ok": true }` 或 `{ "ok": true, "refreshed": false }`
...
```json
{
  "app":     "recipe-one",
  "version": "1.0",
  "prompt_contributions":  [ /* 见 §5.6 — 根级领域提示，勿用已弃用的 system_prompt */ ],
...
> Coding agent 专题：[`spec/tool-responses.md`](spec/tool-responses.md)
...
仅当 widget manifest 的 `display_mode` 为 `chat` 时适用。
...
> Coding agent 专题：[`spec/sessions.md`](spec/sessions.md)
...
**能力地图 / 目录预览（§5.2 optional `preview`）**：Host 调用 `previewAippWidget` — 若模块导出
`preview` 则调用之，否则带 `__preview: true` 回退 `mount`；全程使用 preview `hostApi`，并施加
超时与递归挂载保护（例如禁止预览 `sys.capability-tree` 自身）。
...
| Widget | ✅ | `type`(非 `sys.*`), `app_id`, `is_main`, `display_mode`, `source`, `description` |
| Widget 非 `sys.*` | ✅ | `render` |
...
放在 `aipp-protocol` 模块，AIPP 应用的测试类直接复用。Coding agent 操作指南：[`spec/verify.md`](spec/verify.md)。
...
- ❌ 新 app 仍写 nested `scope.level` / `visible_when` → 用 `visibility` + `owner_widget` / `router_shortcut`（legacy `scope` 仅兼容旧 app）。
...
> **写给 LLM 的最后一句话**：
> 你看完本文档应当能：(1) 实现 4 个 HTTP 端点；(2) 写出合规的 Tool / Skill / Widget JSON；(3) 把多步事务写成 SKILL.md（progressive disclosure）；(4) 用 §6 的 6 个字段自描述所有"想让 Host 帮你特化处理"的行为，**绝不要求修改 Host 代码**；(5) 在需要确认/选择/进度时用 [`spec/system-widgets.md`](spec/system-widgets.md) 中的 `sys.*` 类型；(6) 用 [`spec/capability-tree.md`](spec/capability-tree.md) 理解 Host 如何发现你的 tool/skill。
