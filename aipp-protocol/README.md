...
**与 §5.5 的区别**：§5.5 的 `display_name` / `display_label_zh` 仅用于 **tool/skill 在工具字典中的标签**。本节字段用于 **session、world event、widget 卡片**；新协议**不再**用 `display_name` 描述 widget/event/session 标题（旧字段 `tags.display_name` 仅作 Host 读兼容）。
